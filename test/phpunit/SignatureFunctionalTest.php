...
/**
 * \file    test/phpunit/SignatureFunctionalTest.php
 * \ingroup signature
 ...
 */
class SignatureFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSignature')->click();
		} else {
			// Disable the module
			$this->byHref('modSignature')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/signature/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('signature/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/signature/admin/about.php');
		$this->authenticate();
		return $this->assertContains('signature/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('signaturewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSignature_SignatureTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSignature_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
